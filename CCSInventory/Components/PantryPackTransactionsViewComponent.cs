...
namespace CCSInventory.Components
{
    public class PantryPackTransactionsViewComponent : ViewComponent
    {
        ...
        public IViewComponentResult Invoke(int inOrOut)
        {
            ...
            if(inOrOut == 0)
            {
                List<PantryPackTransaction> transactions = _context.PantryPackTransactions.Include(t => t.PantryPackType).ToList(); 
            ...
            {
                List<PantryPackTransaction> transactions = _context.PantryPackTransactions.Where(t => t.Qty > 0).Include(t => t.PantryPackType).ToList();
